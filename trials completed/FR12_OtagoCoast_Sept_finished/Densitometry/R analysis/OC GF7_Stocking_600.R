...
# Plot 34 - Individually graphs pith to bark density for each tree for 1 plot
ggplot(subset(GF7_Stocking_500new, Plot==34), aes(x=Year, y=Ring_Den, colour=as.factor(Tree), fill=as.factor(Tree))) + 
  geom_line() +
  facet_wrap(. ~ Tree) +
  theme_bw() +
  geom_smooth(method = "lm", alpha=0.1) + #aplha controls transparancy
  labs(colour = "Tree", fill = "Tree") +
  labs(x = "Year", y = "Ring mean density", title = "Plot 34")

# Plot 34 - Produces averages and stdevs by ring for ring density and upper and lower confidence limits
avg34 <- ddply(.data=subset(GF7_Stocking_500new, Plot==34), .(Year), .fun=summarise, RingDen = mean(Ring_Den))
(avg34)
avg34sd <- ddply(.data=subset(GF7_Stocking_500new, Plot==34), .(Year), .fun=summarise, SD = sd(Ring_Den))[,2]# [,2] makes it a vec
avg34upper <- avg34$RingDen + avg34sd 
avg34lower <- avg34$RingDen - avg34sd 
head(avg34sd)



# Plot 34 - Provides graph of average ring mean density for plot with upper and lower levels for sd
ggplot(avg34, aes(x=Year, y=RingDen)) + coord_cartesian(ylim = c(300, 550)) + theme_bw() +
  scale_x_continuous(breaks=seq(1988,2020,2)) + 
  geom_line(colour='black', size=1, linetype='dashed') +
  geom_ribbon(aes(x=Year, ymin=avg34lower, ymax=avg34upper), avg34, alpha=0.2, colour='red', fill='green') +
  #facet_wrap(. ~ Tree) +
  #geom_smooth(method = "lm", alpha=0.2) +
  labs(colour = "Tree", fill = "Tree") +
  labs(x = "Year", y = "Ring mean density", title = "Plot 34 ring mean density")
...
print(ggplot(avg34, aes(x=Year, y=RingDen,)) +
