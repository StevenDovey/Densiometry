...
ggplot(subset(GF7_Stocking_400new, Plot==20), aes(x=Year, y=Ring_Den, colour=as.factor(Tree), fill=as.factor(Tree))) + 
  geom_line() +
  facet_wrap(. ~ Tree) +
  theme_bw() +
...
avg20 <- ddply(.data=subset(GF7_Stocking_400new, Plot==20), .(Year), .fun=summarise, RingDen = mean(Ring_Den))
(avg20)
avg20sd <- ddply(.data=subset(GF7_Stocking_400new, Plot==20), .(Year), .fun=summarise, SD = sd(Ring_Den))[,2]# [,2] makes it a vec
...
ggplot(avg20, aes(x=Year, y=RingDen)) + coord_cartesian(ylim = c(300, 550)) + theme_bw() +
  scale_x_continuous(breaks=seq(1988,2020,2)) + 
  geom_line(colour='black', size=1, linetype='dashed') +
  geom_ribbon(aes(x=Year, ymin=avg20lower, ymax=avg20upper), avg20, alpha=0.2, colour='red', fill='green') +
  #facet_wrap(. ~ Tree) +
...
ggplot(subset(GF7_Stocking_400new, Plot==24), aes(x=Year, y=Ring_Den, colour=as.factor(Tree), fill=as.factor(Tree))) + 
  geom_line() +
  facet_wrap(. ~ Tree) +
  theme_bw() +
  geom_smooth(method = "lm", alpha=0.1) + #aplha controls transparancy
...
avg24 <- ddply(.data=subset(GF7_Stocking_400new, Plot==24), .(Year), .fun=summarise, RingDen = mean(Ring_Den))
(avg24)
avg24sd <- ddply(.data=subset(GF7_Stocking_400new, Plot==24), .(Year), .fun=summarise, SD = sd(Ring_Den))[,2]# [,2] makes it a vec
...
ggplot(avg24, aes(x=Year, y=RingDen)) + coord_cartesian(ylim = c(300, 550)) + theme_bw() +
  scale_x_continuous(breaks=seq(1988,2020,2)) + 
  geom_line(colour='black', size=1, linetype='dashed') +
...
avg2024 <- ddply(.data=subset(GF7_Stocking_400new, Plot==20|24), .(Plot,Year), .fun=summarise, RingDen = mean(Ring_Den),SD=sd(Ring_Den))
avg2024$Plot<-as.character(avg2024$Plot)
avg2024$upper <- avg2024$RingDen + avg2024$SD 
avg2024$lower <- avg2024$RingDen - avg2024$SD 
head(avg2024)

ggplot(avg2024, aes(x=Year, y=RingDen, group=Plot)) + 
  scale_x_continuous(breaks=seq(1988,2020,2)) + 
...
avgGF7_400 <- ddply(.data=avg2024, .(Year), .fun=summarise, RingDen = mean(RingDen))
